...
/**
 * Fetches a list of students who are marked as absent for today.
 * @returns A promise that resolves to an array of student UIDs.
 */
export const getAbsentStudentsToday = async (): Promise<string[]> => {
  try {
    ...
    querySnapshot.forEach(docSnap => {
      const data = docSnap.data() as AttendanceData;
      if (data.studentId && !absentStudentIds.includes(data.studentId)) {
        absentStudentIds.push(data.studentId);
      }
    });
    return absentStudentIds;
  } catch (error: any) {
    console.error("Error fetching absent students for today:", error);
    return [];
  }
};
