...
export default function AttendanceScreen() {
  ...
  const [teacherClasses, setTeacherClasses] = useState<SchoolClass[]>([]); // Use SchoolClass interface
  ...
  const [allSchoolClasses, setAllSchoolClasses] = useState<SchoolClass[]>([]); // New state for all school classes for admin/headteacher filters
  ...
  const fetchTeacherClasses = useCallback(async () => {
    ...
    try {
      const fetchedClasses = await getAllClasses(); // Fetch all classes
      let filteredClasses: SchoolClass[] = [];

      if (userRole === 'teacher') {
        // For a teacher, filter classes they are assigned to (assuming SchoolClass has a teacherId field)
        filteredClasses = fetchedClasses.filter(cls => cls.teacherId === user.uid);
      } else if (userRole === 'admin' || userRole === 'headteacher') {
        // For admin/headteacher, they can see all classes
        filteredClasses = fetchedClasses;
      }
      
      setTeacherClasses(filteredClasses);
      // Also set all school classes for filter options, if applicable (e.g., for admin/headteacher)
      setAllSchoolClasses(fetchedClasses);
  ...
  const fetchStudentsInClass = useCallback(async (classId: string) => {
    ...
    try {
      ...
      if (userRole === 'teacher') {
        ...
        const allStudentsForTeacher = await getStudentsByTeacher(user.uid);
        fetchedStudents = allStudentsForTeacher.filter(s => s.classId === classId); // Further filter by classId
      } else if (userRole === 'admin' || userRole === 'headteacher') {
        // Admins/Headteachers see all students in the selected class
        fetchedStudents = await getStudentsByClass(classId);
      }
    ...
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.actionsHeader}>
          ...
          <TouchableOpacity style={styles.actionButton} onPress={() => fetchAttendanceRecords(false)}>
