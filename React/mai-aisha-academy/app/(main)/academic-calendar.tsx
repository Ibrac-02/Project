...
import { AcademicEvent, AcademicEventData, createAcademicEvent, deleteAcademicEvent, getAcademicEvents, getMalawiHolidaysForYear, updateAcademicEvent } from '../../lib/academicCalendar';
...
export default function AcademicCalendarScreen() {
  ...
  const fetchEvents = useCallback(async () => {
    setPageLoading(true);
    if (user?.uid && role) {
      const fetchedAcademicEvents = await getAcademicEvents(user.uid, role);
      const currentYear = new Date().getFullYear();
      const malawiHolidays = getMalawiHolidaysForYear(currentYear);

      // Combine academic events and holidays
      const allEvents = [...fetchedAcademicEvents, ...malawiHolidays.map(h => ({ ...h, id: `holiday-${h.startDate.toDate().toISOString()}` as string }))];
      ...
      allEvents.forEach((event) => {
        ...
        while (currentDate <= endDate) {
          ...
          newMarkedDates[dateString].dots.push({ color: event.type === 'holiday' ? 'red' : 'blue', selectedDotColor: event.type === 'holiday' ? 'darkred' : 'darkblue' });
