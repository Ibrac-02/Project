...
export default function GradeReportsScreen() {
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      if (downloadURL) {
        Alert.alert("Report Generated and Uploaded", `Report available at: ${downloadURL}`);
        if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(downloadURL);
      }
    } catch (e) {
      console.error("Error generating or uploading report:", e);
      Alert.alert("Error", "Failed to generate or upload report.");
    } finally {
      setIsSharing(false);
    }
  };
