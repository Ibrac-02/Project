...
export const createLessonPlan = async (data: CreateLessonPlanData): Promise<LessonPlan> => {
  try {
    ...
    // Ensure activities contain IDs to satisfy LessonPlan type
    const activitiesWithId: LessonActivity[] = data.activities.map((a, index) => ({
    ...
    return { id: docRef.id, ...data, activities: activitiesWithId, createdAt: now, updatedAt: now, status: 'draft' } as LessonPlan;
  } catch (error: any) {
...
/**
 * Fetches all lesson plans with no filters.
 */
