...
export default function AdminTeacherEdit() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!u) { Alert.alert('Not found', 'Teacher does not exist'); router.back(); return; }
      setUser(u);
      setName(u.name || '');
      setEmail(u.email || '');
  ...
  const confirmDelete = () => {
    if (!user) return;
    Alert.alert('Delete teacher', `Delete ${user.name || user.email}?`, [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: async () => { await deleteUser(user.uid); router.replace('/(admin)/teachers'); } }
    ]);
  };
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.formRow}>
        ...
        <TextInput value={name} onChangeText={setName} placeholder="Teacher name" style={styles.input} />
      </View>
      <View style={styles.formRow}>
      ...
      <View style={styles.formRow}>
        ...
        <TextInput value={department} onChangeText={setDepartment} placeholder="e.g. Mathematics" style={styles.input} />
      </View>
      <View style={styles.formRow}>
        <Text style={styles.label}>Title</Text>
        <TextInput value={title} onChangeText={setTitle} placeholder="e.g. Senior Teacher" style={styles.input} />
      </View>
