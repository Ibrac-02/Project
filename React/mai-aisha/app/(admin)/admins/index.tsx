...
export default function AdminsList() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await listUsersByRole('admin');
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  const confirmDelete = (u: UserProfile) => {
    Alert.alert('Delete admin', `Delete ${u.name || u.email}?`, [
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topBar}>
        ...
        <TouchableOpacity style={styles.addBtn} onPress={() => router.push({ pathname: '/(admin)/admins/new' as any })}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.row}>
              ...
              <TouchableOpacity onPress={() => router.push({ pathname: '/(admin)/admins/[id]', params: { id: item.uid } as any } as any)} style={styles.rowBtn}>
