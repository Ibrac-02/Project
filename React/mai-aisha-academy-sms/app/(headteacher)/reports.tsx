...
export default function HeadteacherReportsScreen() {
  ...
  const [selectedStudentForPerformance, setSelectedStudentForPerformance] = useState<string>('');
  const [selectedClassForPerformance, setSelectedClassForPerformance] = useState<string>('');
  const [selectedTeacherForPerformance, setSelectedTeacherForPerformance] = useState<string>('');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setAllStudents(studentsData); // Students come from students collection
      ...
      const classPerformancePromises = classesData.map(async cls => {
        ...
        const performance = await getClassAveragePerformance(studentIdsInClass, cls.id);
  ...
  const shareReport = async (fileUri: string, fileName: string) => {
    ...
    try {
      ...
      if (fileUri.startsWith('http')) {
        const downloadPath = FileSystem.documentDirectory ? `${FileSystem.documentDirectory}${fileName}` : `${fileName}`;
  ...
  const handleGenerateAttendanceReport = async () => {
    ...
    try {
      const reportUrl = await generateAndUploadAttendanceReport(
        { startDate: attendanceStartDate, endDate: attendanceEndDate, classId: selectedAttendanceClassId || undefined, teacherId: selectedAttendanceTeacherId || undefined },
  ...
  const handleGenerateStudentPerformanceReport = async () => {
    ...
    try {
      const reportUrl = await generateAndUploadStudentPerformanceReport(
        selectedStudentForPerformance || undefined, grades, users, subjects
      );
      if (reportUrl) {
  ...
  const handleGenerateClassPerformanceReport = async () => {
    ...
    try {
      const reportUrl = await generateAndUploadClassPerformanceReport(
        selectedClassForPerformance || undefined, grades, users, allClasses, subjects
      );
      if (reportUrl) {
  ...
  const handleGenerateTeacherPerformanceReport = async () => {
    ...
    try {
      const reportUrl = await generateAndUploadTeacherPerformanceReport(
        selectedTeacherForPerformance || undefined, grades, users, subjects, allClasses
      );
      if (reportUrl) {
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => setSelectedAttendanceClassId(String(itemValue))}
                style={styles.picker}
              >
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => setSelectedAttendanceTeacherId(String(itemValue))}
                style={styles.picker}
              >
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => setSelectedStudentForPerformance(String(itemValue))}
                style={styles.picker}
              >
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => setSelectedClassForPerformance(String(itemValue))}
                style={styles.picker}
              >
                <Picker.Item label="All Classes" value="" />
                {allClasses.map(classItem => (
          ...
          <View style={styles.formGroup}>
            ...
            <View style={styles.pickerContainer}>
              <Picker
                ...
                onValueChange={(itemValue) => setSelectedTeacherForPerformance(String(itemValue))}
                style={styles.picker}
              >
                <Picker.Item label="All Teachers" value="" />
                {allTeachers.map(teacher => (
