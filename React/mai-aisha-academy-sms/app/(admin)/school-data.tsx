...
import { createAcademicYear, createClass, createDepartment, createTerm, deleteAcademicYear, deleteClass, deleteDepartment, deleteTerm, getAllAcademicYears, getAllClasses, getAllDepartments, getAllTerms, getSchoolInfo, updateAcademicYear, updateClass, updateDepartment, updateSchoolInfo, updateTerm } from '../../lib/schoolData';
import { createSubject, deleteSubject, getAllSubjects, updateSubject } from '../../lib/subjects';
import { AcademicYear, Department, SchoolClass, SchoolInfo, Subject, Term, UserProfile } from '../../lib/types';
...
export default function ManageSchoolDataScreen() {
  ...
  const [staffUsers, setStaffUsers] = useState<UserProfile[]>([]); // New state for staff users (teachers/headteachers)
  const [schoolInfo, setSchoolInfo] = useState<SchoolInfo | null>(null); // New state for school info
  ...
  // New state for Department Modal
  const [isDepartmentModalVisible, setDepartmentModalVisible] = useState(false);
  const [currentDepartment, setCurrentDepartment] = useState<Department | null>(null);
  const [departmentName, setDepartmentName] = useState('');
  const [departmentDescription, setDepartmentDescription] = useState('');
  const [departmentHeadId, setDepartmentHeadId] = useState(''); // To store Head of Department UID

  // New state for School Info fields
  ...
  const [selectedTeachers, setSelectedTeachers] = useState<string[]>([]); // For multi-select teachers

  const fetchData = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const [fetchedClasses, fetchedYears, fetchedTerms, fetchedDepartments, allUsers, fetchedSchoolInfo, fetchedSubjects] = await Promise.all([
        getAllClasses(),
        getAllAcademicYears(),
        getAllTerms(),
        getAllDepartments(),
        getAllUsers(),
        getSchoolInfo(),
        getAllSubjects(), // Fetch subjects
      ]);
      const filteredStaff = allUsers.filter(user => user.role === 'teacher' || user.role === 'headteacher');

      setClasses(fetchedClasses);
      setAcademicYears(fetchedYears);
      setTerms(fetchedTerms);
      setDepartments(fetchedDepartments); // Set departments state
      setStaffUsers(filteredStaff); // Set staff users state
      setSubjects(fetchedSubjects); // Set subjects state
      if (fetchedSchoolInfo) {
        setSchoolInfo(fetchedSchoolInfo);
        setSchoolName(fetchedSchoolInfo.name || '');
        setSchoolMotto(fetchedSchoolInfo.motto || '');
        setSchoolLogoUrl(fetchedSchoolInfo.logoUrl || '');
        setSchoolAddress(fetchedSchoolInfo.address || '');
        setSchoolContactEmail(fetchedSchoolInfo.contactEmail || '');
        setSchoolContactPhone(fetchedSchoolInfo.contactPhone || '');
      }
  ...
  const handleEditDepartment = (item: Department) => {
    ...
    setDepartmentDescription(item.description || '');
    setDepartmentHeadId(item.headOfDepartmentId || '');
    setDepartmentModalVisible(true);
  };
  ...
  const handleSaveDepartment = async () => {
    ...
    try {
      if (currentDepartment) {
        await updateDepartment(currentDepartment.id, { name: departmentName, description: departmentDescription, headOfDepartmentId: departmentHeadId });
        Alert.alert('Success', `Department "${currentDepartment.name}" updated successfully.`);
      } else {
        await createDepartment(departmentName, departmentDescription, departmentHeadId);
        Alert.alert('Success', `Department "${departmentName}" created successfully.`);
      }
      setDepartmentModalVisible(false);
      fetchData();
    } catch (err: any) {
      Alert.alert('Error', `Failed to save department: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveSchoolInfo = async () => {
    ...
    setLoading(true);
    try {
      const updates: Partial<SchoolInfo> = {
        name: schoolName,
        motto: schoolMotto,
      ...
      await updateSchoolInfo(updates);
      Alert.alert('Success', 'School information updated successfully!');
      fetchData();
    } catch (err: any) {
      Alert.alert('Error', `Failed to update school information: ${err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEditSubject = (item: Subject) => {
    ...
    setSelectedTeachers(item.teachers || []);
    setSubjectModalVisible(true);
  };
  ...
  const handleSaveSubject = async () => {
    ...
    try {
      if (currentSubject) {
        await updateSubject(currentSubject.id, { name: subjectName, description: subjectDescription, teachers: selectedTeachers });
        Alert.alert('Success', `Subject "${currentSubject.name}" updated successfully.`);
      } else {
        await createSubject(subjectName, subjectDescription, selectedTeachers);
        Alert.alert('Success', `Subject "${subjectName}" created successfully.`);
      }
  ...
  const renderDepartmentItem = ({ item }: { item: Department }) => (
    <View style={styles.card}>
      <Text style={styles.cardTitle}>Department: {item.name}</Text>
      {item.description && <Text style={styles.cardDetail}>{item.description}</Text>}
      {item.headOfDepartmentId && <Text style={styles.cardDetail}>HOD ID: {item.headOfDepartmentId}</Text>}
      <View style={styles.cardActions}>
        <TouchableOpacity onPress={() => handleEditDepartment(item)} style={styles.actionButton}>
          <Ionicons name="pencil-outline" size={24} color="#4CAF50" />
        </TouchableOpacity>
        <TouchableOpacity onPress={() => handleDeleteDepartment(item.id, item.name)} style={styles.actionButton}>
          <Ionicons name="trash-outline" size={24} color="#F44336" />
        </TouchableOpacity>
      </View>
    </View>
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <TextInput
          style={styles.input}
          placeholder="School Logo URL (Optional)"
          value={schoolLogoUrl}
          onChangeText={setSchoolLogoUrl}
        />
        <TextInput
          style={styles.input}
        ...
        <TextInput
          ...
          onChangeText={setSchoolContactEmail}
        ...
        <TextInput
          ...
          onChangeText={setSchoolContactPhone}
          keyboardType="phone-pad"
        />
        <TouchableOpacity style={styles.saveButton} onPress={handleSaveSchoolInfo}>
          <Text style={styles.buttonText}>Save School Info</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        {departments.length === 0 ? (
          <Text style={styles.noDataText}>No departments found.</Text>
        ) : (
          <FlatList
            data={departments}
            keyExtractor={(item) => item.id}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>{currentDepartment ? 'Edit Department' : 'Add New Department'}</Text>
            <TextInput
              style={styles.input}
              placeholder="Department Name (e.g., Science, Arts)"
            ...
            />
            <Picker
              selectedValue={departmentHeadId}
              onValueChange={(itemValue) => setDepartmentHeadId(itemValue)}
              style={styles.picker}
            >
              <Picker.Item label="-- Select Head of Department (Optional) --" value="" />
              {staffUsers.map(user => (
                <Picker.Item key={user.uid} label={user.name || user.email || user.uid} value={user.uid} />
              ))}
            </Picker>
            <View style={styles.modalButtons}>
              <TouchableOpacity style={[styles.modalButton, styles.cancelButton]} onPress={() => setDepartmentModalVisible(false)}>
                <Text style={styles.buttonText}>Cancel</Text>
              </TouchableOpacity>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            />
            <Picker
              selectedValue={selectedTeachers.length > 0 ? selectedTeachers[0] : ''}
              onValueChange={(itemValue) => setSelectedTeachers(itemValue ? [itemValue] : [])}
              style={styles.picker}
            >
              <Picker.Item label="-- Select Teacher (Optional) --" value="" />
              {staffUsers.map(user => (
                <Picker.Item key={user.uid} label={user.name || user.email || user.uid} value={user.uid} />
              ))}
            </Picker>
            {/* Note: Multi-select for teachers would require a custom component or checkbox list. */}
            <View style={styles.modalButtons}>
              <TouchableOpacity style={[styles.modalButton, styles.cancelButton]} onPress={() => setSubjectModalVisible(false)}>
                <Text style={styles.buttonText}>Cancel</Text>
              </TouchableOpacity>
