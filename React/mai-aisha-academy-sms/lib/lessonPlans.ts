...
export const createLessonPlan = async (data: CreateLessonPlanData): Promise<LessonPlan> => {
  try {
    ...
    // Ensure activities have IDs to match LessonPlan type
    const activitiesWithIds: LessonActivity[] = data.activities.map((act, index) => ({
      id: Date.now().toString() + '-' + index + Math.random().toString(36).slice(2, 8),
      title: act.title,
    ...
    return { id: docRef.id, ...data, activities: activitiesWithIds, createdAt: now, updatedAt: now, status: 'draft' } as LessonPlan;
  } catch (error: any) {
...
/**
 * Fetches all lesson plans (for headteacher/admin views)
 */
export const getAllLessonPlans = async (): Promise<LessonPlan[]> => {
  try {
    const lessonPlanCollection = collection(db, 'lessonPlans');
    const querySnapshot = await getDocs(lessonPlanCollection);
    const lessonPlans: LessonPlan[] = [];
    querySnapshot.forEach((doc) => {
      lessonPlans.push({ id: doc.id, ...(doc.data() as Omit<LessonPlan, 'id'>) });
    });
    return lessonPlans;
  } catch (error: any) {
...
export const getLessonPlanStats = async (teacherId: string) => {
  try {
    const lessonPlans = await getTeacherLessonPlans(teacherId);
