...
export const updateGrade = async (id: string, updates: Partial<Omit<Grade, 'id' | 'createdAt' | 'gradePercentage'>>): Promise<void> => {
  try {
    ...
    if (updates.marksObtained !== undefined || updates.totalMarks !== undefined) {
      // Fetch current grade doc to calculate new percentage if only one field is updated
      const currentDoc = await getDocs(query(collection(db, "grades"), where("__name__", "==", id)));
      const currentData = currentDoc.docs[0]?.data() as Grade | undefined;

      const marksObtained = updates.marksObtained ?? currentData?.marksObtained ?? 0;
      const totalMarks = updates.totalMarks ?? currentData?.totalMarks ?? 0;

      gradeUpdates.gradePercentage = totalMarks > 0
